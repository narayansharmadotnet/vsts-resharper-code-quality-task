...
Write-Output $inspectCodeExePath

$tempDownloadFolder = [Environment]::GetEnvironmentVariable('BUILD_STAGINGDIRECTORY')
...
if(!(Test-Path $inspectCodeExePath)) {
    ...
    Write-Verbose "root path '$PSScriptRoot'"
    ...
    Write-Verbose "nugetExeLocation passed to Start-Process is '$nugetExeLocation' - working directory - '$tempDownloadFolder'"
    ...
    $inspectCodeExePath =  [System.IO.Path]::Combine($commandLineInterfacePath, "inspectcode.exe");
}
...
Write-Verbose "Invoking InspectCode.exe from - '$inspectCodeExePath' using arguments '$arguments'"
